...
function SaveJson($data) {
	// if (!$data) { return; }
	$path = PATH_SET.$data['account']."/";
	$fpath = GetPostKey('fpath'); // 追加-指定目錄
	if ($fpath) { $path .= $fpath; }
	// txt('SaveJson-path: ', $path); // @@
...
function Get_JosFile($data) {
	...
	$filename = (!$path ? PATH_JSON : $path."/").$data['fname'].".json";
	// $filename = $path.$data['fname'];

	include FN_FILE;
	// txt('Get_JosFile: ', $data);	/* @QQ: Test開關 */
	$obj1 = get_json($filename, $path);
	txt('Get_JosFile: ', $obj1);	/* @QQ: Test開關 */
	return $obj1;
}
...
/* 取得該路徑下所有目錄及檔案 */
function Get_AllPathFile($data) {
	// txt('Get_AllFiles_byPath: ', $data);	// @@
	// $path = $data['path'];
	$path = PATH_SET.$data['account']."/";
	$fpath = GetPostKey('fpath'); // 追加-指定目錄
	if ($fpath) { $path .= $fpath; }
	...
	$dirs = get_dirs($path);
	// txt('dirs: ', $dirs);	// @@
	foreach(($dirs ?: []) as $i => $dir1) {
		$path1 = $path.'/'.$dir1;
		// txt('path1: ', $path1);	// @@
		$files = get_file_dir($path1, false);	// 參2:副檔名否
		$result[] = [
			'dir'      => $dir1,
